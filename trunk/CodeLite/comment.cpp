...
, m_line( line )
{
	m_comment.erase(m_comment.find_last_not_of(wxT("\n\r\t"))+1);
}
...
{
	return wxT("DELETE FROM COMMENTS WHERE File=? AND Line=?");
}

wxString Comment::GetUpdateOneStatement()
{
	return wxT("UPDATE COMMENTS SET Comment=? WHERE File=? AND Line=?");
}

wxString Comment::GetInsertOneStatement()
{
	return wxT("INSERT INTO COMMENTS VALUES (?, ?, ?)");	
}
} // codelite
