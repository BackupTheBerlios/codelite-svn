...
, m_parseComments(false)
{
	...
	m_ctagsCmd[TagsGlobal] = wxT("  --language-force=C++ --fields=aKmSsnit --c-kinds=+p --C++-kinds=+p --filter=yes --filter-terminator=\"<<EOF>>\" ");
	m_ctagsCmd[TagsLocal] =  wxT("  --language-force=C++ --fields=aKmSsnit --c-kinds=+l --C++-kinds=+l --filter=yes --filter-terminator=\"<<EOF>>\" ");
}  
...
TagTreePtr TagsManager::ParseTagsFile(const wxFileName& fp, const wxString& project)
{
	...
	if( !file ) 
	{
		DEBUG_MSG(wxT("cant open tag file!"));
...
TagTreePtr TagsManager::ParseSourceFile(const wxFileName& fp, const wxString& project, std::vector<DbRecordPtr> *comments)
{
	...
	if(m_ctags == NULL)
	{
		DEBUG_MSG(wxT("ctags process, for global scope, is not running, call TagsManager::StartCtagsProcess()"));
		return TagTreePtr( NULL );
	}
...
std::vector<TagEntry>* TagsManager::ParseLocals(const wxString& scope)
{
	...
	{
		DEBUG_MSG(wxT("ctags process, for local scope, is not running, call TagsManager::StartCtagsProcess()"));
		return NULL;
	}
...
TagsProcess* TagsManager::StartCtagsProcess(wxEvtHandler* evtHandler, int id, int kind)
{
	...
	{
		DEBUG_MSG(wxT("Faild to start ctags: Invalid ctags kind"));
		return NULL;
	}
	
	// build the command, we surround ctags name with double quatations
	cmd << wxT("\"") << m_ctagsPath.GetFullPath() << wxT("\"") << iter->second;
	TagsProcess* process;
	...
	if( process->GetPid() <= 0 )
	{
		DEBUG_MSG(wxT("Failed to start ctags process"));
		return NULL;
	}
...
void TagsManager::SourceToTags(const wxFileName& source, wxString& tags, TagsProcess* ctags)
{
	...
	{
		DEBUG_MSG(wxT("Failed to connect to ctags process's standard input"));
		return;
	}
...
void TagsManager::GetTags(const wxString& name, const wxString& scopeName, std::vector<TagEntry>& tags, int flags, const wxString& scope, bool allowDups)
{
	...
	DEBUG_START_TIMER(wxT("Quering database with the following sql: ") << query)
	wxSQLite3ResultSet rs = m_pDb->Query(query);
	DEBUG_STOP_TIMER();
	...
	{
		DEBUG_START_TIMER(wxT("Quering external database with the following sql: ") << query);
		ex_rs = m_pExternalDb->Query(query);
		DEBUG_STOP_TIMER();
	}

	// Add the database results
	DEBUG_START_TIMER(wxT("Populating map with database results ... "));
	...
	// Sort the results base on their name
	DEBUG_START_TIMER(wxT("Sorting result vector ..."))
	std::sort(tags.begin(), tags.end(), SDescendingSort());	
	DEBUG_STOP_TIMER()
}
...
void TagsManager::BuildExternalDatabase(const wxFileName& rootDir, const wxFileName &dbName, const wxString& WXUNUSED(language), wxWindow* updateDlgParent/** null */)
{
	...
	for(i=0; i<maxVal; i++)
	{
		...
		{
			wxString msg;
			msg << wxT("File: ") << curFile.GetFullPath();
			prgDlg->Update(i, msg);
		}
