...
void TagsDatabase::CreateSchema()
{
	...
	try
	{
		...
		sql = wxT("create  table if not exists tags (project string, name string, file string, line integer, kind string, access string, signature string, pattern string, parent string, inherits string, path string, typeref string);");
		m_db->ExecuteUpdate(sql);
		...
		sql = wxT("CREATE UNIQUE INDEX IF NOT EXISTS TAGS_UNIQ on tags(project, kind, path, signature);");
		m_db->ExecuteUpdate(sql);
		...
		sql = wxT("create table if not exists tags_version (version string);");
		m_db->ExecuteUpdate(sql);
...
wxSQLite3ResultSet TagsDatabase::SelectTagsByProject(const wxString& project, const wxFileName& path)
{
	...
	if(project.IsEmpty())
	{
		query = wxString::Format(wxT("select * from tags;"));
	}
	else
	{
		query << wxT("select * from tags where project='") << project << wxT("';");
	}
...
wxSQLite3ResultSet TagsDatabase::SelectTagsByProjectAndFile(const wxString& project, const wxString& file, const wxFileName& path)
{
	...
	wxString query;
	query = wxString::Format(wxT("select * from tags where project='%s' and file='%s';"), project.GetData(), file.GetData());
	return m_db->ExecuteQuery(query.GetData());
}
...
void TagsDatabase::Delete(const wxFileName& path, const wxString& project, const wxString& fileName, bool autoCommit)
{
	...
	try	
	{
		if( autoCommit )
			m_db->Begin();
		m_db->ExecuteUpdate(wxString::Format(wxT("Delete from tags where project='%s' AND File='%s'"), project.GetData(), fileName.GetData()));
...
void TagsDatabase::LoadToMemory(const wxFileName& fn)
{
	...
	try
	{
		...
		m_db->Begin();
		sql = wxT("insert into tags select project, name, file, line, kind, access, signature, pattern, parent, inherits, path, typeref FROM backup.tags");
		m_db->ExecuteUpdate(sql);
		m_db->Commit();
	}
	catch(wxSQLite3Exception& e)
