...
public:
	...
	bool End();
	void operator++(int);
//	TreeNode<TKey, TData>* GetNode() { return m_current; }
...
{
//	return (m_current == m_root && !m_current->HasMore());
...
/*	while( true )
	{
		TreeNode<TKey, TData>* child = m_current->Next();
		if( child )
		{
			// This node has more childs
			m_current = child;
			m_current->First();
			return;
		}
		...
		{
			// No more childs for this node, check if we are the root, and if so, end the scan
			if(m_current == m_root)
				return;
			else
			{
				// We are not the root, so get our parent
