...
, m_tipKind(TipNone)
{
	...
	m_propertyInt[wxT("wxSCI_C_DEFAULT")] = 0;
	m_propertyInt[wxT("wxSCI_C_COMMENT")] = 1;
	m_propertyInt[wxT("wxSCI_C_COMMENTLINE")] = 2;
	m_propertyInt[wxT("wxSCI_C_COMMENTDOC")] = 3;
	m_propertyInt[wxT("wxSCI_C_NUMBER")] = 4;
	m_propertyInt[wxT("wxSCI_C_WORD")] = 5;
	m_propertyInt[wxT("wxSCI_C_STRING")] = 6;
	m_propertyInt[wxT("wxSCI_C_CHARACTER")] = 7;
	m_propertyInt[wxT("wxSCI_C_UUID")] = 8;
	m_propertyInt[wxT("wxSCI_C_PREPROCESSOR")] = 9;
	m_propertyInt[wxT("wxSCI_C_OPERATOR")] = 10;
	m_propertyInt[wxT("wxSCI_C_IDENTIFIER")] = 11;
	m_propertyInt[wxT("wxSCI_C_STRINGEOL")] = 12;
	m_propertyInt[wxT("wxSCI_C_VERBATIM")] = 13;
	m_propertyInt[wxT("wxSCI_C_REGEX")] = 14;
	m_propertyInt[wxT("wxSCI_C_COMMENTLINEDOC")] = 15;
	m_propertyInt[wxT("wxSCI_C_WORD2")] = 16;
	m_propertyInt[wxT("wxSCI_C_COMMENTDOCKEYWORD")] = 17;
	m_propertyInt[wxT("wxSCI_C_COMMENTDOCKEYWORDERROR")] = 18;
	m_propertyInt[wxT("wxSCI_C_GLOBALCLASS")] = 19;
...
void LEditor::SetProperties()
{
	...
	wxFileName ConfigFileName(wxT("liteeditor.xml"));
	...
	if (ConfigFileFound)
		EditorConfigST::Get()->LoadWords(ConfigFileName,  wxT("LEX_CPP"), keyWords);
	...
	if (ConfigFileFound)
		EditorConfigST::Get()->LoadStyle(ConfigFileName, wxT("LEX_CPP"), styles);
...
void LEditor::SaveFile()
{
	...
	//-------------------------------------------------------------------
	wxFFile file(m_fileName.GetData(), wxT("wb"));
	if(file.IsOpened() == false)
	{
		// Nothing to be done
		wxString msg = wxString::Format(wxT("Failed to open file %s"), m_fileName.GetData());
		wxMessageBox( msg );
		return;
	}
...
void LEditor::OpenFile(const wxString &fileName, const wxString &project)
{
	...
	wxFFile file(fileName.GetData(), wxT("r"));
	if(file.IsOpened() == false)
	{
		// Nothing to be done
...
void LEditor::CompleteWord()
{
	...
	DEBUG_START_TIMER(wxT("Scanning for tags ..."))
...
void LEditor::CodeComplete()
{
	...
	// the first '{' or ';' or SOT
	int semiColPos = FindString(wxT(";"), 0, false, GetCurrentPos());
	int lcurlyPos  = FindString(wxT("{"), 0, false, GetCurrentPos());
	int start;
	TagEntry tag;
	int line = 1;
...
void LEditor::GotoDefinition()
{
	...
	wxString logmsg;
	logmsg << wxT("Searching for symbol: ") << word;
	...
	else
	{
		...
		for(size_t i=0; i<tags.size(); i++)
			logmsg << wxT("\n") << tags[i].GetName() << wxT(" : ") << tags[i].GetFile() << wxT(" : ") << tags[i].GetLine();
		wxLogMessage(logmsg);
#endif
	}
}
...
void LEditor::OnDwellStart(wxScintillaEvent & event)
{
	...
	// We define an expression, by reading from pos and up until we find
	// the first '{' , ';' or SOT
	int semiColPos = FindString(wxT(";"), 0, false, end);
	int lcurlyPos  = FindString(wxT("{"), 0, false, end);
	semiColPos > lcurlyPos ? start = semiColPos : start = lcurlyPos;
