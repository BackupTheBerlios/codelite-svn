...
wxFNBCustomizeDialog::wxFNBCustomizeDialog( wxWindow* parent, int id, wxString title, wxPoint pos, wxSize size, int style ) : wxDialog( parent, id, title, pos, size, style )
{
...
wxPanel *wxFNBCustomizeDialog::CreateOptionsPage()
{
	...
	m_foreignDnD->Enable(m_allowDragAndDrop->IsChecked());
