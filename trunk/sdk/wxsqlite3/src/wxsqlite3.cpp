...
const wxChar* wxERRMSG_NODB          = _("No Database opened");
const wxChar* wxERRMSG_NOSTMT        = _("Statement not accessible");
const wxChar* wxERRMSG_NOMEM         = _("Out of memory");
const wxChar* wxERRMSG_DECODE        = _("Cannot decode binary");
const wxChar* wxERRMSG_INVALID_INDEX = _("Invalid field index");
const wxChar* wxERRMSG_INVALID_NAME  = _("Invalid field name");
const wxChar* wxERRMSG_INVALID_ROW   = _("Invalid row index");
const wxChar* wxERRMSG_INVALID_QUERY = _("Invalid scalar query");

const wxChar* wxERRMSG_NORESULT      = _("Null Results pointer");
const wxChar* wxERRMSG_BIND_STR      = _("Error binding string param");
const wxChar* wxERRMSG_BIND_INT      = _("Error binding int param");
const wxChar* wxERRMSG_BIND_INT64    = _("Error binding int64 param");
const wxChar* wxERRMSG_BIND_DBL      = _("Error binding double param");
const wxChar* wxERRMSG_BIND_BLOB     = _("Error binding blob param");
const wxChar* wxERRMSG_BIND_DATETIME = _("Error binding date/time param");
const wxChar* wxERRMSG_BIND_NULL     = _("Error binding NULL param");
const wxChar* wxERRMSG_BIND_CLEAR    = _("Error clearing bindings");
...
const unsigned char* wxSQLite3FunctionContext::GetBlob(int argIndex, int& len)
{
  if (argIndex >= 0 && argIndex < m_argc)
  {
    if (!IsNull(argIndex))
    {
      ...
      return (const unsigned char*)sqlite3_value_blob((sqlite3_value*) m_argv[argIndex]);
    }
    else
    {
      return NULL;
...
{ _T("SQLITE_COPY"),              _T("SQLITE_CREATE_INDEX"),      _T("SQLITE_CREATE_TABLE"),
  ...
  _T("SQLITE_REINDEX"),           _T("SQLITE_ANALYZE")
};
